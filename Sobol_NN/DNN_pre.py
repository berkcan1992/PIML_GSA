...
def pass_arg(Xx, nsim, tr_size):
    ...
    def PGNN_train_test(optimizer_name, optimizer_val, drop_rate, iteration, n_layers, n_nodes, tr_size):
    ...
    #     batch_size = int(tr_size/2)
        ...
        # Load labeled data
        data = np.loadtxt('../data/labeled_data.dat').astype(np.float32)
        x_labeled = data[:, :2]
    ...
    #     y_labeled = scaler.fit_transform(y_labeled)
        ...
        # train and test data
        trainX, trainY = x_unlabeled[:tr_size,:], y_unlabeled[:tr_size]
